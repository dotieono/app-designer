...
function addMenuButton(key, value, label, divToAddButtonTo) {
    ...
    if (key === util.leafRegion) {
        ...
        button.on('click', function () {
            var queryParams = util.getKeyToAppendToColdChainURL(key, value);
            odkTables.launchHTML(null,'config/assets/region.html' + queryParams);
        });
    } else {
        button.on('click', function () {
...
function showSubregionButtonsAndTitle(jsonRegion) {
    ...
    if (jsonRegion.hasOwnProperty('sector')) {
        var sector = jsonRegion.sector;
        for (var subRegCtr = 0; subRegCtr < sector.length; subRegCtr++) {
            var sector = sector[subRegCtr];
            var sectorLabel = sector.label;
            var sectorToken = sector.token;
            var localizesector = odkCommon.localizeText(locale, sectorToken);
            if (localizesector !== null && localizesector !== undefined) {
                sectorLabel = localizesector;
            }
            addMenuButton(util.region, sector.region, sectorLabel, '#buttonsDiv');
        }
